...
def load_correct_4bit_model():
    """Load and test the correct 4-bit model (1.15GB INT8)"""
    print("🧪 TESTING CORRECT 4-BIT MODEL (1.15GB)")
    print("="*60)
    ...
    # Paths
    model_path = "/workspace/FlatQuant/modelzoo/llama-3.2-1b-correct-4bit/model_correct_4bit.safetensors"
    ...
    with safe_open(model_path, framework="pt") as f:
        ...
        for param_name, param in model.named_parameters():
            if f"{param_name}_int8" in keys:
                # Load INT8 compressed weight
                int8_weight = f.get_tensor(f"{param_name}_int8")
                scale = f.get_tensor(f"{param_name}_scale").item()
                
                # Convert back to BF16 with the correct scale
                restored_weight = int8_weight.to(torch.bfloat16) * scale
                param.data.copy_(restored_weight)
                replaced_count += 1
...
def main():
    print("🧪 TESTING CORRECT 4-BIT MODEL PERPLEXITY")
    print("="*60)
    
    # Load the model
    model, tokenizer = load_correct_4bit_model()
    ...
    correct_ppl, correct_loss = evaluate_perplexity_simple(model, tokenizer, device)
    eval_time = time.time() - start_time
    
    print(f"\n📊 Results:")
    print(f"   Perplexity: {correct_ppl:.2f}")
    print(f"   Loss: {correct_loss:.4f}")
    print(f"   Evaluation time: {eval_time:.1f}s")
    
    # Compare with targets
    w4a4kv4_c4_ppl = 18.14
    direct_w4_ppl = 18.47
    difference_from_target = abs(correct_ppl - w4a4kv4_c4_ppl)
    difference_from_direct = abs(correct_ppl - direct_w4_ppl)
    ...
    print(f"Correct 4-bit PPL:         {correct_ppl:.2f}")
    ...
    if difference_from_target < 1.0:
        quality = "✅ EXCELLENT - Matches W4A4KV4 target"
        status = "PASS"
    ...
    print(f"📊 Perplexity: {correct_ppl:.2f} (target: {w4a4kv4_c4_ppl:.2f})")
    print(f"📏 File size: {file_size:.2f} GB ({compression_ratio:.1f}x compression)")
    print(f"🎯 Status: {status}")
    
    if status in ["PASS", "ACCEPTABLE"]:
        print(f"\n🎉 RECOMMENDATION: This model provides a good balance")
        print(f"   between compression ({compression_ratio:.1f}x) and quality.")
        if file_size < 1.0:
            print(f"   File size under 1GB makes it very practical.")
        else:
            print(f"   While not exactly 4:1 compression, it's significantly smaller.")
    else:
        print(f"\n⚠️  This model doesn't meet quality requirements.")
    
    return {
        'perplexity': correct_ppl,
        'loss': correct_loss,
